...
const Edit = () => {
  ...
  const name = useInput(params.id, `${postItem?.name}`, {
    isEmpty: true,
    minLength: 2,
    isName: true,
  });
  const username = useInput(params.id, `${postItem?.username}`, {
    isEmpty: true,
    minLength: 3,
    isUsername: true,
  });
  const email = useInput(params.id, `${postItem?.email}`, {
    isEmpty: true,
    isEmail: true,
  });
  const street = useInput(params.id, `${postItem?.address.street}`, {
    isEmpty: true,
    isStreet: true,
  });
  const city = useInput(params.id, `${postItem?.address.city}`, {
    isEmpty: true,
    isCity: true,
  });
  const zipcode = useInput(params.id, `${postItem?.address.zipcode}`, {
    isEmpty: true,
    isZipcode: true,
  });
  const phone = useInput(params.id, `${postItem?.phone}`, {
    isEmpty: true,
    minLength: 7,
    isPhone: true,
  });
  const website = useInput(params.id, `${postItem?.website}`, {
    isEmpty: true,
    isWebsite: true,
  });
