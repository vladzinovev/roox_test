...
const Navigation = () => {
  ...
  function handleClick(name: string) {
    ...
    name==='city'?dispatch(setArrowCity(!arrowCity)):dispatch(setArrowCompany(!arrowCompany))    
  }
  ...
  return (
    <div className={styles.navigation}>
      ...
      <button className={styles.btn_sort} onClick={() => handleClick("city")}>
        по городу {arrowCity?<img alt="up" src={up}/>:<img alt="up" src={down}/>}
      </button>
      <button className={styles.btn_sort} onClick={() => handleClick("company")}>
        по компании {arrowCompany?<img alt="up" src={up}/>:<img alt="up" src={down}/>}
      </button>
    </div>
  );
};
